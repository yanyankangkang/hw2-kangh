...
public class Evaluator extends JCasAnnotator_ImplBase {
  ...
  private HashMap<String, Lingpipe> mLingDict = new HashMap<String, Lingpipe>();

  // private EntrezGeneDAO eg = null;
  /**
   * this parameter is generated by another CPE procedure
   */
 private double mTheta[] = { -9.009515604622354, 14.819006159685497, 8.36333671635934 };
 
  
  /*  private double mTheta[] = {21.24961194160029, //Map
          -142.223105371483,
          -5.210242613646323,
          104.13137242863395,
          81.65884166026315,
  ...
  public void initialize(JCas jcas) {
    ...
    while (Lingpipe_it.hasNext()) {
      ...
      mLingDict.put(Ling.getWords(), Ling);
    }

    mModel = new Logisitic(2);
   ...
   * @param JCas fecth Abner annotation and LingPipe annotation from aJCas 
   */
  public void process(JCas aJCas) throws AnalysisEngineProcessException {
    ...
    while (abner_it.hasNext()) {
      ...
      Lingpipe Ling = mLingDict.get(word);
      if (mLingDict.containsKey(word)) {
        x[0] = Ling.getConfidence();
        mLingDict.remove(word);
      } else {
        x[0] = 0;
      }
      if (mModel.Classify(x, mTheta)) {
    ...
    for (String word : mLingDict.keySet()) {
      Lingpipe Ling = mLingDict.get(word);
